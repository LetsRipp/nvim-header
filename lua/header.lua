...
local function update_time()
    -- Get the first few lines of the buffer
    local bufnr = vim.api.nvim_get_current_buf()
    local lines = vim.api.nvim_buf_get_lines(bufnr, 0, 10, false)
    
    -- Get the comment symbol for current file
    local symbol, md = get_extension()
    ...
    for i, line in ipairs(lines) do
        if line:match("Date:") or line:match("Created:") then
            date_found = true
            date_line_idx = i - 1  -- Convert to 0-based index
            break
        end
    end
    
    if not date_found then
        vim.notify("No date field found in header", vim.log.levels.WARN)
        return
    end
    
    -- Format the new date line
    local new_date = os.date("%Y-%m-%d")
    local new_line
    if md then
        new_line = symbol .. "Last updated: " .. new_date .. ")"
    else
        new_line = symbol .. " Last updated: " .. new_date
    end
    
    -- Update the line
    vim.api.nvim_buf_set_lines(bufnr, date_line_idx, date_line_idx + 1, false, {new_line})
end
